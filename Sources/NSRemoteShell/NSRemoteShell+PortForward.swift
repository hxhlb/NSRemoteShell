...
extension NSRemoteShell {
    ...
    func removeForwardTask(id: UUID) {
        forwardTasks.removeValue(forKey: id)
    }
}

private extension NSRemoteShell {
    static func forwardLog(_ message: String) {
    ...
    func runLocalForward(
        ...
        while await !state.isCancelled(), shouldContinue(), isConnected, !Task.isCancelled {
            let ready = await (try? SocketPoller.waitAsync(
                socket: listenSocket,
                events: [.read],
                timeout: SSHConstants.socketWaitSlice
            )) ?? false
            if !ready {
                continue
            }
            ...
            Self.forwardLog("local forward accepted client")
            ...
            do {
                ...
                Self.forwardLog("local forward direct channel opened to \(targetHost):\(targetPort)")
                Task {
                    await self.bridge(
                        session: session,
                        channel: channel,
                ...
                Self.forwardLog("local forward channel open failed error=\(error)")
    ...
    func runRemoteForward(
        ...
        while await !state.isCancelled(), shouldContinue(), isConnected, !Task.isCancelled {
            ...
            Self.forwardLog("remote forward accepted channel")
            ...
            guard let socket else {
                Self.forwardLog("remote forward target connect failed to \(targetHost):\(targetPort)")
                await closeChannelAsync(session: session, channel)
                continue
            }
            Self.forwardLog("remote forward connected to \(targetHost):\(targetPort)")
            Task {
                await self.bridge(
                    session: session,
                    channel: channel,
        ...
        // Async cancel instead of synchronous busy-loop
        do {
            _ = try await session.retrying(
                timeout: SSHConstants.operationTimeout,
                operation: libssh2_channel_forward_cancel(listener),
    ...
    func openRemoteListener(session: SSHSession, remotePort: Int) async throws -> (OpaquePointer, Int) {
        ...
        while true {
            ...
            let lastErrno = session.withLock {
    ...
    func pumpSocketToChannel(
        ...
        while await !state.isCancelled(), shouldContinue(), isConnected, !Task.isCancelled {
            ...
            if recvCount > 0 {
                Self.forwardLog("socket->channel bytes=\(recvCount)")
                do {
                    try await writeChannelBytes(session: session, channel: channel, buffer: buffer, count: Int(recvCount))
                } catch {
                    Self.forwardLog("socket->channel write failed error=\(error)")
                    break
                }
                continue
            }
            if recvCount == 0 {
                Self.forwardLog("socket closed")
                break
            }
            let code = errno
            if code == EAGAIN || code == EWOULDBLOCK {
                _ = try? await SocketPoller.waitAsync(socket: socket, events: [.read], timeout: SSHConstants.socketWaitSlice)
                continue
            }
            Self.forwardLog("socket recv failed errno=\(code) message=\(String(cString: strerror(code)))")
            break
        }
    }
    ...
    func pumpChannelToSocket(
        ...
        while await !state.isCancelled(), shouldContinue(), isConnected, !Task.isCancelled {
            do {
                ...
                if channelRead > 0 {
                    Self.forwardLog("channel->socket bytes=\(channelRead)")
                    var sent = 0
                    while sent < channelRead {
                        ...
                        let code = errno
                        if code == EAGAIN || code == EWOULDBLOCK {
                            _ = try await SocketPoller.waitAsync(socket: socket, events: [.write], timeout: SSHConstants.socketWaitSlice)
                            continue
                        }
                        Self.forwardLog("channel->socket send failed errno=\(code) message=\(String(cString: strerror(code)))")
                        sent = channelRead
                        break
                    }
                    continue
                }
                Self.forwardLog("channel closed")
                break
            } catch {
                Self.forwardLog("channel read failed error=\(error)")
                break
            }
        }
    }
