...
enum SocketUtilities {
    ...
    static func createConnectedSocket(host: String, port: Int, nonBlocking: Bool) throws -> Int32 {
        ...
        while true {
            ...
            // Set non-blocking BEFORE connect so connect() itself is non-blocking
            if nonBlocking {
                do {
                    try setNonBlocking(socketFd)
                } catch {
            ...
            if address.ai_family == AF_INET {
                ...
                connectResult = withUnsafePointer(to: &ipv4) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                ...
                connectResult = withUnsafePointer(to: &ipv6) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
            ...
            // Non-blocking connect returns EINPROGRESS — this is expected
    ...
    /// Wait for a non-blocking connect to complete. Returns true if connected, false on timeout.
    static func waitForConnect(socket: Int32, timeout: TimeInterval) async throws -> Bool {
        let ready = try await SocketPoller.waitAsync(socket: socket, events: [.write], timeout: timeout)
        guard ready else { return false }

        var error: Int32 = 0
        var length = socklen_t(MemoryLayout<Int32>.size)
        let result = getsockopt(socket, SOL_SOCKET, SO_ERROR, &error, &length)
        guard result == 0 else {
            throw RemoteShellError.socketError(code: errno, message: String(cString: strerror(errno)))
        }
        guard error == 0 else {
