...
func withFileTransfer<T>(shell: NSRemoteShell, _ operation: @Sendable () async throws -> T) async throws -> T {
    try await shell.connectFileTransfer()
    do {
        let result = try await operation()
        await shell.disconnectFileTransfer()
        return result
    } catch {
        await shell.disconnectFileTransfer()
        throw error
...
func runLargeEchoClient(
    ...
    do {
        ...
        let connected = try await SocketUtilities.waitForConnect(socket: socket, timeout: timeout)
        guard connected else {
            print("runLargeEchoClient: connect timed out port=\(port)")
            SocketUtilities.closeSocket(socket)
    ...
    while sent < sizeBytes || received.count < sent, !cancelled {
        ...
        if sent >= sizeBytes, received.count < sent {
            _ = try? await SocketPoller.waitAsync(socket: socket, events: [.read], timeout: 0.1)
        }
    }
...
func runLocalEchoClient(port: Int, payload: String, timeout: TimeInterval = 10) async throws -> Bool {
    ...
    do {
        ...
        let connected = try await SocketUtilities.waitForConnect(socket: socket, timeout: timeout)
        guard connected else {
            print("runLocalEchoClient: connect timed out port=\(port)")
            SocketUtilities.closeSocket(socket)
            return false
        }
    } catch {
    ...
    while sent < expected.count {
        ...
        if sendError == EAGAIN || sendError == EWOULDBLOCK {
            ...
            _ = try await SocketPoller.waitAsync(socket: socket, events: [.write], timeout: 1)
            continue
        }
    ...
    while received.count < expected.count {
        ...
        if readError == EAGAIN || readError == EWOULDBLOCK {
            ...
            _ = try await SocketPoller.waitAsync(socket: socket, events: [.read], timeout: 1)
            continue
        }
